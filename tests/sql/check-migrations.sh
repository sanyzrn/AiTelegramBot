#!/usr/bin/env bash
# Applies tests/sql/stubs.sql and every migration in order to an empty
# PostgreSQL, twice (idempotency), then runs the behavioural smoke test.
# Usage: PGHOST=… PGPORT=… PGUSER=… PGDATABASE=… tests/sql/check-migrations.sh
set -euo pipefail
cd "$(dirname "$0")/../.."
run() { psql -v ON_ERROR_STOP=1 -q "$@"; }
run -f tests/sql/stubs.sql
for pass in 1 2; do
