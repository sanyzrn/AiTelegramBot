...
- **Better answers.** One rewritten system prompt (`_shared/tone.ts`): answer first, honest about uncertainty and live data, never claims a reminder/expense/message was saved from chat (points to the exact phrase that does it), Telegram-safe formatting (no Markdown tables), per-length guidance and a real guide for every tone (friendly, formal, romantic, professional, creative were previously unguided). Tool prompts for calc, email, summary, translation, rewrite, ideas and the fun pack were rewritten; text tools now get explicit instructions instead of the raw text.
- **Fixes.** Calculations the exact engine cannot handle («اگه ماهی ۱۲ میلیون پس‌انداز کنم…») go to the step-by-step calculator instead of a refusal; a failed request sends one message (precise reason + retry keyboard) instead of two; text quoted by a reply is kept for image/OCR tools; the menu-button sync no longer delays the first reply after a cold start; a prompt that merely mentions «خلاصه» is no longer forced into a monospace block; calculator answers render as normal text; the expense parser no longer glues Persian letters to digits («۲پاکت شیر»); clearer shopping/briefing hints.
- **Dashboard redesign.** Bottom tab bar, gradient hero card with a working mini chart (it was drawn into a `<div>` and never showed), tappable stat cards, quick-action shortcuts, colored categories with percentages, skeleton loading, visible 14-day chart (SVG gradient fix), Persian error messages, inputs cleared only after a successful save, one all-or-nothing `expenses_add` batch for quick-add, «فردا» labels, dark-mode fallback and reduced-motion support. Separators no longer look like the Persian zero.
