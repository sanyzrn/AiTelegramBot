...
async function send(chat, text, page = null, id = chat) {
...
async function sendSpoiler(chat, q, a) {
  await send(chat, q);
  await tg("sendMessage", {
    chat_id: chat,
...
async function deliver(chat, text, tool, prompt) {
  const parts = [],
    rx = /```([A-Za-z0-9_+#-]*)[ \t]*\r?\n([\s\S]*?)\r?\n?```/g;
  let end = 0,
    m;
...
async function configSet(key, val) {
...
async function show(id, chat, page) {
...
async function adminInput(id, chat, text) {
...
async function exportMd(id, chat) {
  const { data, error } = await db
...
async function documentSend(chat, body, name) {
...
async function navigate(id, chat, text, p) {
  const pages = {
    "🏠 خانه": "home",
    "🧰 ابزارها": "tools",
    "⚙️ تنظیمات": "settings",
...
async function file(id, size, max = 7000000) {
...
async function ai(s, contents, system) {
...
async function metric(update, id, s, status, usage = {}, reason = "") {
...
async function startWork(m, update, tool, prompt, override = null) {
  const id = m.from.id,
    chat = m.chat.id,
    original = update,
    s = await cfg(),
...
async function profile(id, chat) {
...
async function retry(id, chat, update) {
  const { data: item, error } = await db
    .from("saeed_ai_retry")
