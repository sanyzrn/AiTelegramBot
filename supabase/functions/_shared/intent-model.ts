...
const hint = /(?:یاد|ریم[ای]ندر|فردا|ساعت|دقیقه|امروز|هفته|خبر|قیمت|نرخ|جست|سرچ|آنلاین|بگرد|گوگل|گیت|ریپو|مخزن|خلاصه|ترجمه|بازنویسی|محاسبه|حساب|ایمیل|تسک|وظیفه|کارام|کارها|ایده|خرج|هزینه|ناهار|بنزین|خرید|صبح.نامه|شهر\s*من|شهرم|شهر\s*صبح.نامه|آب|هوا|طلا|سکه|دلار|یورو|بورس|مسکن|بیت|نتیجه|امتیاز|برنده|رئیس|چه\s*کسی|remind|tomorrow|search|latest|news|price|weather|today|current|who\s+(?:won|is)|github|summari[sz]e|translate|rewrite|calculate|email|task|expense|shopping|briefing|\/city)/iu;
