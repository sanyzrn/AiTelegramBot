...
export function inferToolIntent(text: string): ToolIntent {
  ...
  if (/^(?:\/expense|\/expenses|\/spend|خرج\s+|خرج[‌\s-]*هام|گزارش\s+خرج)/iu.test(input) || isExpenseCommand(input) || /^(?:ناهار|شام|صبحانه|بنزین|تاکسی|اسنپ|قهوه|خوراک|خرید|قبض|کرایه|اجاره|دارو|نان|سوخت|پارکینگ|سوپرمارکت|میوه|رستوران|کافه|فروشگاه)\s+[۰-۹٠-٩\d,٬،]+(?:\s*(?:هزار|میلیون))?\s*(?:تومان|تومن|ریال)?$/iu.test(input)) return "expenses";
  ...
  if (/(?:خلاصه[‌\s]*(?:اش|ش|اشو|شو|شون|هاش)?\s*کن|خلاصه\s*کن|summari[sz]e)/iu.test(input)) return "summarize";
  if (/(?:ترجمه[‌\s]*(?:اش|ش|اشو|شو|شون|هاش)?\s*کن|ترجمه\s*کن|translate)/iu.test(input)) return "translate";
  if (/(?:بازنویسی[‌\s]*(?:اش|ش)?\s*کن|بازنویسی\s*کن|rewrite)/iu.test(input)) return "rewrite";
